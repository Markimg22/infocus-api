...
describe('DbCreateUser UseCase', () => {
  ...
  it('should call CreatePerformanceRepository with correct userId', async () => {
    const { sut, createPerformanceRepositorySpy, createUserRepositorySpy } =
      makeSut();
    await sut.create(mockCreateUserParams());
    expect(createPerformanceRepositorySpy.userId).toBe(
      createUserRepositorySpy.result
    );
  });
  ...
  it('should throws if CreatePerformanceRepository throws', async () => {
    ...
    jest
      .spyOn(createPerformanceRepositorySpy, 'create')
      .mockImplementationOnce(throwError);
    const promise = sut.create(mockCreateUserParams());
    await expect(promise).rejects.toThrow();
  });
